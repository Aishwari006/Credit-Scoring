...
# production-safe cleaning
X_rank = X_rank.replace([np.inf, -np.inf], np.nan)
X_rank = X_rank.fillna(X_rank.median())
...
for col in X_rank.columns:
    try:
        auc = roc_auc_score(y_train, X_rank[col])
...
X_train = X_train[model_input_columns]
X_test = X_test[model_input_columns]
...
# =========================================================
# FINAL PREPROCESSING (FIT ONLY ON SELECTED FEATURES)
# =========================================================
...
# =========================================================
# SAVE PRODUCTION SCHEMA (CRITICAL — DO NOT MODIFY ORDER)
# =========================================================


# force pure python list (prevents pandas index serialization bugs in production)
joblib.dump(list(model_input_columns), "05_model_input_columns.pkl")


# save fallback values for production scoring
feature_defaults = X_train_imp.median().to_dict()
joblib.dump(feature_defaults, "08_feature_defaults.pkl")


# debugging safety — confirms mapping_layer and training match
...
joblib.dump(coefficients, "09_model_coefficients.pkl")
