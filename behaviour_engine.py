...
def load_transactions(path):
    ...
    if df["date"].isna().sum() > 0:
        raise Exception("Date parsing failed. Ensure DD/MM/YYYY format")
...
def behavioural_features(df):
    ...
    features["credit_std"] = monthly_credit.std()
    features["credit_stability"] = features["avg_monthly_credit"] / (features["credit_std"] + EPS)

    if len(monthly_credit) > 1:
        features["credit_trend"] = (
            (monthly_credit.iloc[-1] - monthly_credit.iloc[0]) /
            (monthly_credit.iloc[0] + EPS)
        )
    else:
    ...
    # ---------------- SHOCK EVENTS ----------------
    q = 0.95 if len(g) > 40 else 0.85
    large_debit = g["debit"] > g["debit"].quantile(q)
    features["large_debit_events"] = large_debit.sum()

    rolling_mean = daily_balance.rolling(7, min_periods=1).mean()
    balance_drop = daily_balance < 0.7 * rolling_mean
